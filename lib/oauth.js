...
export async function handleOAuthRequest(req, res, url) {
  ...
  if (req.method === "POST" && url.pathname === "/oauth/register") {
    try {
      ...
      oauthError(res, 400, "invalid_client_metadata", error.message);
    }
    return true;
  }
  ...
  if (req.method === "GET" && url.pathname === "/oauth/authorize") {
    try {
      await validateAuthorizeParams(url.searchParams);
      res.writeHead(200, {
  ...
  if (req.method === "POST" && url.pathname === "/oauth/authorize") {
    ...
    try {
      ...
      if (!safeEqual(ownerCode, env("OWNER_CODE"))) {
        ...
        res.end(authorizePage(body, "The owner code is not correct."));
        return true;
      }
      ...
      res.writeHead(302, { location: redirect.toString(), "cache-control": "no-store" }).end();
    } catch (error) {
      oauthError(res, 400, "invalid_request", error.message);
    }
    return true;
